<template>
    <AuthenticatedLayout title="Travaux" selected-menu="Travaux">
...
const refresh = () => {
    ...
    router.get(route('Travaux.index'));
};
...
const editRow = (record) => {
    router.get(route('Travaux.edit', {page: record}, '/edit'));
}
    ...
    Modal.confirm({
        ...
        onOk() {
            return new Promise((resolve, reject) => {
                router.delete(route('Travaux.destroy', {page: record.id}), {
                    preserveScroll: true,
                    onSuccess: page => {
                        resolve()
...
const handleTableChange = (pag, filters, sorter) => {
    router.get(route('Travaux.index', {page: pag.current}));
};
